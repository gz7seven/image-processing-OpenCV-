...
{  
    char* filename = argc >= 2 ? argv[1] : (char*)"C:/AIPCV/4.jpg";  
    CvMemStorage* storage = cvCreateMemStorage(0);  //创建动态存储，用来创建一个内存存储器，来统一管理各种动态对象的内存。
    CvRNG rng = cvRNG(-1);  
    ...
    marker_mask = cvCreateImage( cvGetSize(img), 8, 1 ); //创建首地址并分配存储空间 IplImage* cvCreateImage( CvSize size, int depth, int channels );
    markers = cvCreateImage( cvGetSize(img), IPL_DEPTH_32S, 1 );  
    cvCvtColor( img, marker_mask, CV_BGR2GRAY );//得到灰度图  
    ...
    {  
        ...
        {  
            ...
            cvFindContours( marker_mask, storage, &contours, sizeof(CvContour),  
                CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE );//函数cvFindContours从二值图像中检索轮廓，并返回检测到的轮廓的个数  
            cvZero( markers );  
